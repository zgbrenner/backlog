...
| `TIMEOUT:exceeded <n>s at stage <stage>` | One file used up its whole time budget (180 seconds by default). Very large PDFs and slow scans do this. **Use Try again.** If it happens on many files, ask IT to raise `per_file_wall_clock_secs` in `backlog.config.json`. |
...
| `RECOVERED:planned quarantine` | BackLog restarted after it had planned this file's move to Quarantine but before its review record was complete. It recovered that safe, unfinished step instead of silently delivering or deleting the file. **Open Needs Review and decide what to do with the file there.** |
| `RUNTIME_FAIL:filter …` / `RUNTIME_FAIL:name reservation …` / `RUNTIME_FAIL:manifest …` / `RUNTIME_FAIL:routing task panicked …` | The failure was in BackLog or the computer, not in your document — a folder disappeared, a disk filled up, or a part of BackLog stopped answering. **Open Settings and press Check this computer, fix anything it reports, then Try again.** A Power Automate delivery may be at the handoff step; a Local folder delivery may be at the local output and receipt step. |
...
| `DISMISSED:<your note>` | Not a failure. Someone pressed **Can't fix this** on this file and confirmed the **Set aside for good?** prompt, so BackLog recorded a *decision* rather than work completed. The card leaves Needs Review and the file stays untouched in Quarantine for manual handling. In Power Automate mode Flow 2 can record a dismissed handoff; Local folder mode does not send a handoff. **This is final inside BackLog.** There is no button anywhere in the app that brings it back — **Try again** only helps *before* you confirm, while the card is still on screen. If you change your mind afterwards, file it by hand or ask for it to be re-delivered **under a different name**. (Dropping the same file back into Processing does nothing: BackLog recognises it and leaves it alone. Under a different name it is treated as a second copy.) |
...
| `SUBJECT_TRUNCATED` | The suggested subject ran past the sixty-four-word limit a filename can carry, so it was cut to the first sixty-four words — the document type, form number and party, which is what a filename is for — and any trailing separator left dangling by the cut was removed too. A trim, never an addition: nothing was invented, and the full wording is still in the file's description. |
...
| `DESCRIPTION_DATE_REMOVED` | The suggested description repeated a calendar date even though dates belong only at the start of the filename. BackLog removed that date phrase, rechecked the complete sentence, and filed it only after every description rule still passed. |
...
| `outbox_not_writable` | In **Power Automate / SharePoint** mode, BackLog could not create the Outbox handoff location for Flow 2. **Check that the selected Outbox folder is available, writable, and has free disk space; use Create this folder for me or Browse if offered, then press Check this computer.** |
| `local_output_not_writable` | In **Local folder** mode, BackLog could not create a test file in Local Output, so it cannot deliver a finished renamed document or its receipt. **Check that the selected folder is available, writable, and has free disk space; use Create this folder for me or Browse if offered, then press Check this computer.** |
| `local_output_no_create_new_publication` | The selected Local Output folder cannot safely publish a new file without risking an overwrite. **Choose a local NTFS folder for Local Output, or ask IT to enable hard links there; then press Check this computer.** |
| `quarantine_not_writable` / `cache_not_writable` | BackLog could not create a test file in that required folder — no permission, a full disk, or a OneDrive folder that is still syncing. **Check the folder opens in Explorer and that the disk is not full, then press Check this computer.** |
| `models_missing` | The everyday 0.6B naming model is missing. A normal installer and portable ZIP include it. **Press the download action BackLog shows, or reinstall BackLog if the bundled model was removed.** If the optional backup model is also absent, the combined download is about 2.5 GB; otherwise BackLog downloads only the everyday model, about 0.6 GB. No account is needed. |
| `escalation_model_missing_using_primary` (warning) | The optional 1.7B backup model is not installed, or this computer is deliberately not set up for one — computers with about 8 GB of memory reuse the everyday model for backup naming attempts on purpose. **Nothing is broken: BackLog is ready and safely reuses the everyday model for difficult naming attempts.** If you want the larger backup model and have about 1.8 GB of disk space, press **Download optional backup model**. The transfer can be cancelled and resumed. |
