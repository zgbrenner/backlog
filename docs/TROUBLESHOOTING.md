...
| `models_missing` | The everyday 0.6B naming model is missing. A normal v0.7.0 installer and portable ZIP include it. **Press the download action BackLog shows, or reinstall v0.7.0 if the bundled model was removed.** If the optional backup model is also absent, the combined download is about 2.4 GB; otherwise BackLog downloads only the everyday model, about 0.6 GB. No account is needed. |
